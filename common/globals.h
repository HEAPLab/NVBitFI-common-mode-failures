...
// __managed__ uint64_t per_sm_counter1[MAX_SMS];
// __managed__ uint64_t per_sm_counter2[MAX_SMS];
// __managed__ uint64_t per_sm_counter3[MAX_SMS];
__managed__ uint64_t per_sm_counters[MAX_SMS][NUM_COUNTERS]; // per-SM instruction counters
__managed__ uint64_t g_target_pc;
...
float get_flip_prob() {
	...
	float r = dist(gen);  // random number between 0 and 1

	//printf("HOST: generated random flip threshold: %f\n", r);
