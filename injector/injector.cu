...
//__managed__ volatile unsigned long long g_target_pc = 0ULL;
...
void parse_params(std::string filename) {
	...
	if (!parse_flag) {
		...
		// On host, after kernel completes (or before next kernel run)
...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
	...
	for (auto f : related_functions) {
		...
		if(strcmp(inj_info.kernelName, kname.c_str()) == 0) { // this is the kernel selected for injection 
			...
			float random_value = get_flip_prob(); // random value for determining whether to inject or not
			float flip_prob = 1.0f; // default injection probability threshold, change if needed

			uint32_t flip_bits, rand_bits;
			memcpy(&flip_bits, &flip_prob, sizeof(flip_bits));
			memcpy(&rand_bits, &random_value, sizeof(rand_bits));
			...
			for (auto i : instrs) {
				...
				nvbit_add_call_arg_const_val64(i, (uint64_t)per_sm_counters);
				nvbit_add_call_arg_const_val64(i, (uint64_t)&g_target_pc);
				...
				nvbit_add_call_arg_const_val32(i, flip_bits); // injection probability threshold
				nvbit_add_call_arg_const_val32(i, rand_bits); // random value for determining whether to inject or not
				// printf("HOST: flip_bits=0x%08x, rand_bits=0x%08x\n", flip_bits, rand_bits);
			}
		} else {
			const std::vector<Instr *> &instrs = nvbit_get_instrs(ctx, f);
			if (verbose)
