    ...
    class MemAllocation {
        ...
        public: 
            ...
            size_t isInMemAllocation(void *ptr) {
                if (data <= ptr && ptr < (void*)(((char*)(data)) + (NUM_REPLICAS)*size)) {
                    return size;
                }
                else {
                    return 0;
	    ...
	    // uses the cnt, incrementing it and returning it
	    int useCnt() {
		 cnt++;
	         return cnt;
	    }
        ...
        if (memcmp(dst, dst_cpy.data(), count) != 0) {
            ...
            for (int i=0; i<count; i++) { 
                if (((char*)(dst))[i] != (dst_cpy)[i]) {
                    ...
                    else if (NUM_REPLICAS == 3) // fai il check anche sulla seconda (opzionale)
		    {
